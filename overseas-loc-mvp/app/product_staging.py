...
def force_stage_product_assets(
    ...
    *,
    ...
    """
    强制刷新固定产品垫图：
    - seedance-source.* ← 白底主图（唯一 I2V 身份锚点）
    - seedance-usage-ref.* ← 倒出口参考（仅 Prompt / 追溯，不作 I2V）
    """
    _ = market
...
def validate_product_staging(project: Path, product_id: str) -> dict[str, Any]:
    ...
    manifest_path = project / "inputs" / "product-staging.json"
    manifest: dict[str, Any] = {}
    if manifest_path.is_file():
        try:
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        except (json.JSONDecodeError, OSError):
            errors.append("product-staging.json 损坏")
    elif is_fixed_product(pid):
        errors.append("固定产品缺少 product-staging.json，需重新注入垫图")
...
def resolved_i2v_image_ref(project: Path, product_id: str) -> str | None:
    """固定产品：所有 AI 分镜 I2V 统一使用已注入的白底主图。"""
    if is_fixed_product(product_id):
        staged = resolve_staged_seedance_source(project)
        if staged and staged.is_file():
            return staged.relative_to(project).as_posix()
    return None


def pour_usage_prompt_suffix(product_id: str, role: str) -> str:
    if not is_fixed_product(product_id):
        return ""
    role = (role or "").strip()
    if role not in ("方案", "证明"):
        return ""
    from .product_usage import THERMOS_POUR_ACTION_EN

    return THERMOS_POUR_ACTION_EN
